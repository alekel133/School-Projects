...
int main(int argc, char *argv[]){
  ...
  // Seed random generator, and generate random port.
  srand(time(nullptr));
  int r_port = rand() % 65536; 
  if(r_port < 1024) {
	 r_port = 1024 + r_port;
  }
  ...
  // Send port to client.
  if (sendto(mysocket, &r_port, sizeof(r_port), 0, (struct sockaddr *)&client, clen)==-1){
    cout << "Error in sendto function.\n";
  }
     
  close(mysocket);

  // Make new socket with r_port
  if((mysocket=socket(AF_INET, SOCK_DGRAM, 0))==-1)
