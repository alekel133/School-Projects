...
__global__
void computeResidual_kerneli(float *presid, float *uresid, float *vresid, float *wresid,
		     const float *p,
		     const float *u, const float *v, const float *w,
		     float eta, float nu, float dx, float dy, float dz,
		     int ni, int nj, int nk, int kstart,
...
__global__
void computeResidual_kernelj(float *presid, float *uresid, float *vresid, float *wresid,
		     const float *p,
		     const float *u, const float *v, const float *w,
		     float eta, float nu, float dx, float dy, float dz,
		     int ni, int nj, int nk, int kstart,
...
__global__
void computeResidual_kernelk(float *presid, float *uresid, float *vresid, float *wresid,
		     const float *p,
		     const float *u, const float *v, const float *w,
		     float eta, float nu, float dx, float dy, float dz,
		     int ni, int nj, int nk, int kstart,
...
//		       ni, nj, nk, kstart, iskip, jskip, L) ;
  ...
  copy_cpu_to_gpu(&p[0], p_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&u[0], u_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&v[0], v_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&w[0], w_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&presid[0], presid_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&uresid[0], uresid_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&vresid[0], vresid_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&wresid[0], wresid_cuda, allocsize, "weightedSum3");
  copy_cpu_to_gpu(&pnext[0], pnext_cuda, allocsize, "weightedSum3"); 
  copy_cpu_to_gpu(&unext[0], unext_cuda, allocsize, "weightedSum3"); 
  copy_cpu_to_gpu(&vnext[0], vnext_cuda, allocsize, "weightedSum3"); 
  copy_cpu_to_gpu(&wnext[0], wnext_cuda, allocsize, "weightedSum3"); 
  ...
  while(simTime < stopTime) {
    ...
    /*copyPeriodic(&p[0],&u[0],&v[0],&w[0],
		 ni, nj, nk, kstart, iskip, jskip) ;*/
    copyPeriodic_kernel<<<max(ni,nj), max(nj,nk)>>>(p_cuda,u_cuda,v_cuda,w_cuda,
		 ni, nj, nk, kstart, iskip, jskip) ;
    ...
    zeroResidual_kernel<<<ni, nj>>>(presid_cuda,uresid_cuda, vresid_cuda, wresid_cuda,
		 ni, nj, nk , kstart, iskip, jskip) ;

    // Compute the residual, these will be used to compute the rates of change
    // of pressure and velocity components

    computeResidual_kerneli<<<ni+1 , nj>>>(presid_cuda, uresid_cuda, vresid_cuda, wresid_cuda,
		    p_cuda, u_cuda, v_cuda, w_cuda,
		    eta, nu, dx, dy, dz,
		    ni, nj, nk, kstart, iskip, jskip) ;
    computeResidual_kernelj<<<ni , nj+1>>>(presid_cuda, uresid_cuda, vresid_cuda, wresid_cuda,
		    p_cuda, u_cuda, v_cuda, w_cuda,
		    eta, nu, dx, dy, dz,
		    ni, nj, nk, kstart, iskip, jskip) ;
    computeResidual_kernelk<<<ni , nj>>>(presid_cuda, uresid_cuda, vresid_cuda, wresid_cuda,
		    p_cuda, u_cuda, v_cuda, w_cuda,
		    eta, nu, dx, dy, dz,
		    ni, nj, nk, kstart, iskip, jskip) ;
    ...
    */
    ...
    // copy data to the ghost cells to implement periodic boundary conditions
    copyPeriodic_kernel<<<max(ni,nj), max(nj,nk)>>>(pnext_cuda,unext_cuda,vnext_cuda,wnext_cuda,
		 ni, nj, nk, kstart, iskip, jskip) ;
    ...
    */
    ...
    copy_gpu_to_cpu(pnext_cuda, &pnext[0], allocsize, "copyPeriodic");
    copy_gpu_to_cpu(unext_cuda, &unext[0], allocsize, "copyPeriodic");
    copy_gpu_to_cpu(vnext_cuda, &vnext[0], allocsize, "copyPeriodic");
    copy_gpu_to_cpu(wnext_cuda, &wnext[0], allocsize, "copyPeriodic");
    copy_gpu_to_cpu(presid_cuda, &presid[0], allocsize, "zeroResidual");
    copy_gpu_to_cpu(uresid_cuda, &uresid[0], allocsize, "zeroResidual");
    copy_gpu_to_cpu(vresid_cuda, &vresid[0], allocsize, "zeroResidual");
    copy_gpu_to_cpu(wresid_cuda, &wresid[0], allocsize, "zeroResidual");
    computeResidual(&presid[0], &uresid[0], &vresid[0], &wresid[0],
		    &pnext[0],&unext[0], &vnext[0], &wnext[0],
		    eta, nu, dx, dy, dz,
		    ni, nj, nk, kstart, iskip, jskip) ;

    copy_cpu_to_gpu(&pnext[0], pnext_cuda, allocsize, "copyPeriodic");
    copy_cpu_to_gpu(&unext[0], unext_cuda, allocsize, "copyPeriodic");
    copy_cpu_to_gpu(&vnext[0], vnext_cuda, allocsize, "copyPeriodic");
    copy_cpu_to_gpu(&wnext[0], wnext_cuda, allocsize, "copyPeriodic");
    copy_cpu_to_gpu(&presid[0], presid_cuda, allocsize, "zeroResidual");
    copy_cpu_to_gpu(&uresid[0], uresid_cuda, allocsize, "zeroResidual");
    copy_cpu_to_gpu(&vresid[0], vresid_cuda, allocsize, "zeroResidual");
    copy_cpu_to_gpu(&wresid[0], wresid_cuda, allocsize, "zeroResidual");
