...
class BVH():
    ...
    #TODO: Work on generating initial box, and then subboxes through Nodes.
    #This will probably be done by generating the bounding box, and then calling
    #genBox for the subboxes. Add list for planes, as we want to remove them from
    #the BVT, as they have infinite bounds.
    def genBox(self):
        maxPoint = (float("-inf", "-inf", "-inf"), float("-inf", "-inf", "-inf"), float("-inf", "-inf", "-inf"))
        minPoint = (float("inf", "inf", "inf"), float("inf", "inf", "inf"), float("inf", "inf", "inf"))
        for item in self.items:
            id = item.id()
            if id == "Sphere":
                xMax, yMax, zMax = item.center[0] + item.radius, item.center[1] + item.radius, item.center[2] + item.radius
                xMin, yMin, zMin = item.center[0] - item.radius, item.center[1] - item.radius, item.center[2] - item.radius
